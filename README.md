...
Mobile devices have limitations such as draw calls, texture size, amount of geometry, and more. To optimize performance, we've kept our scripts and shaders as efficient as possible for our effects productions.
All of the code in this repository is open-source, which means that you can view, modify, and contribute to it in any way you see fit. Thank you for visiting our repository, 
and we hope that these VFX shaders/scripts will help you to create stunning VFX on mobile devices!
<br>
...
## C# Scripts
### Overview
Those C# scripts are created to control and manipulate various VFX elements, such as particle systems, animations, 
and other graphical effects. They can be used to change the behavior and appearance of the VFX, making it possible to create interactive and dynamic experiences for players.
<br>
...
## VFX Render Bounds
### Overview
Obtaining the Render Bounds of a Mesh or Skinned Mesh Renderer and passing them into the Target Material. 
The material requires the following variables: '_BoundsMin, _BoundsMax, _BoundsCtr, _BoundsSize'.
<br>
...
### Overview
This script scales a game object based on the rotation from a another game object.
Apply this script to the game object that requires scaling.
<br>
...
### Overview
This script synchronizes the target particles' position with the source position.
Apply this script on any Game Object. 
### Properties
| Properties             | Description                                         |
|------------------------|-----------------------------------------------------|
...
Except for UI shaders, all other shaders share common rendering properties. 
The following attributes define an object's appearance during rendering.
> **Note**
...
## True Color Shaders
True color shaders perform linear interpolation between two colors, guided by a mask. 
Another feature is to blend between additive and pre-multiply mode, guided by black and white values.
<br>
...
## Image Shaders
### Overview
The image shaders are specially designed to be used by other artists, such as UI or non-technical / VFX artists.
This shader is intended to be used with a UI image.
<br>
...
## Glitch Shader
### Overview
This shader creates a glitch effect, like a TV glitch.
By default, it is set up with textures and a distortion map that creates a cartoon style.
For a different style, simply change the textures.
<br>
...
## Desaturation Shader
### Overview
This shader desaturates the original image.
For performance reasons, this shader doesn't use a typical HSL function.
...
#### Fusion Concept!
The fusion concept essentially involves merging two grayscale ramps and subsequently subtracting the grayscale values with another value. 
After this, the values are multiplied by 100 to regain sharp edges while maintaining smooth, rounded shapes. 
With additional motion applied to each ramp, and this way, you can create a fluid-like animation.
For good results, the grayscale ramp should be soft.
<br>
...
### Overview
This shader combines different dynamics and motion together, for example dissolving, distortions, UV sweeps, etc. The dynamics are controlled by the custom data from the particle system Shuriken. 
You will also need meshes; the UV should be warped in one direction (U or V).
<br>
...
For that reason, I've built these debug properties to provide a better understanding of what the shader is doing. 
These properties generate a new compute shader. After you've finished debugging, don't forget to disable the 'Compute Debug Mode', as this enables and disables all debug modes.
...
### Overview
This shader is a multifunctional for VFX Artists, enabling the creation of diverse and dynamic visual effects through a modular and customizable approach. 
It covers a broad spectrum of VFX needs, from geometry and UV manipulation, lighting and shading, to dissolve and distortion effects.
<br>
...
| Deforming Strength                | Strength of the geometry deformation.                     |
...
| Properties                             | Description                                                |
|----------------------------------------|------------------------------------------------------------|
| Gradient Color                         | Sets the color of the gradient.                            |
| Gradient Strength                      | Controls the visibility of the gradient.                   |
| Gradient Brightness                    | Controls the strength of the adding effect.                |
|                                        |                                                            |
| Gradient by Light                      |                                                            |
|                                        |                                                            |
| Multiply Light Source and Light Radius | Sets the swirl direction.                                  |
| Set Position                           | Sets the local position from asset in game play mode.      |
| Light Position                         | Position of the light relative to the object/particles.    |
| Light Range                            | Controls the light's effect range.                         |
| Light Fall Off                         | Controls the fall-off range.                               |
| Light Hardness                         | Controls the hardness of the fall-off.                     |
|                                        |                                                            |
| Gradient by Uv                         |                                                            |
|                                        |                                                            |
| Uv Ramp Axis Blend                     | Sets the swirl direction.                                  |
| Uv Ramp Offset                         | Blends the different channels' XYZ values of RGB.          |
| Uv Ramp Settings                       | Uses UV tiling and offset to repeat and shift the texture. |
| Light                                  | Stores masks in the channels, used as alpha.               |

### Look And Feel Properties
| Properties                   | Description                                             |
|------------------------------|---------------------------------------------------------|
| Look Up Texture              | Color gradient applied to grayscale alpha texture.      |
| Switch LUT to Color Gradient | Switch between applying a gradient to the UVs or alpha. |
| LUT Use Custom Data 1        | Enables animating the Look Up Texture.                  |
| LUT By CD1 Channel (0 - 3)   | Controls the UV row by custom data 1.                   |

### Alpha Properties
| Properties                              | Description                                                                     |
|-----------------------------------------|---------------------------------------------------------------------------------|
| Packed Texture                          | Flips UV coordinates horizontally.                                              |
| Blend Packed Texture Channel            | Flips UV coordinates vertically.                                                |
| Uv Tiling & Offset                      | Rotates UV coordinates by 90 degrees.                                           |
|                                         |                                                                                 |
| Alpha Mask                              |                                                                                 |
|                                         |                                                                                 |
| Enable Alpha Mask                       | Changes values from ramp; higher values give hard edge results.                 |
| Uv Tiling & Offset                      | Uses UV tiling and offset to repeat and shift the position of the map.          |
| Enable Squeeze XY and Swipe ZW          | Applies gradient information to control the direction of the dissolve effect.   |
| Use Custom Data Channel (0 - 3)         | Uses UV tiling and offset to repeat and shift the position of the map.          |
|                                         |                                                                                 |
| Dissolve Properties                     |                                                                                 |
|                                         |                                                                                 |
| Flip U                                  | Applies distortion to the alpha texture.                                        |
| Flip V                                  | Applies distortion to the dissolve map.                                         |
| Rotate 90 Degrees                       | Applies distortion to the mask map.                                             |
| Invert Ramp                             | Uses UV tiling and offset to repeat and shift the position of the texture.      |
| Use Custom Data                         | Applies distortion to the UVs to create directional distortion effect.          |
| Strength By Custom Data Channel (0 - 3) | Uses UV tiling and offset to repeat and shift the position of the map.          |
| Strength                                | Uses UV tiling and offset to repeat and shift the position of the map.          |
| Uv Tiling & Offset                      | Uses UV tiling and offset to repeat and shift the position of the map.          |
| Fall Off                                | Uses UV tiling and offset to repeat and shift the position of the map.          |

### Distortion Properties
| Properties                                  | Description                                                                  |
|---------------------------------------------|------------------------------------------------------------------------------|
| Texture                                     | Applies distortion to the alpha texture.                                     |
| Mask                                        | Applies distortion to the dissolve map.                                      |
| Mask Invert                                 | Applies distortion to the mask map.                                          |
| Enable For Alpha                            | Uses UV tiling and offset to repeat and shift the position of the texture.   |
| Enable For Dissolve                         | Applies distortion to the UVs to create directional distortion effect.       |
| Use Custom Data 1                           | Uses UV tiling and offset to repeat and shift the position of the map.       |
| Strength By Use Custom Data Channel (0 - 3) | Uses UV tiling and offset to repeat and shift the position of the map.       |
| Strength                                    | Uses UV tiling and offset to repeat and shift the position of the map.       |
| Set Uv To World Coordinates                 | Uses UV tiling and offset to repeat and shift the position of the map.       |
| Uv Tiling & Offset                          | Uses UV tiling and offset to repeat and shift the position of the map.       |
...
| Uv Alpha                                    |                                                                            |
|                                             |                                                                            |
| Enable Squeeze XY and Swipe ZW              | Applies distortion to the alpha texture.                                   |
| Use Custom Data 1 Channel (0 - 3)           |                                                                            |
| Uv Distortion                               |                                                                            |
|                                             |                                                                            |
| Enable Squeeze XY and Swipe ZW              | Applies distortion to the mask map.                                        |
| Use Custom Data 1 Channel (0 - 3)           | Uses UV tiling and offset to repeat and shift the position of the texture. |
| Uv Twist                                    |                                                                            |
|                                             |                                                                            |
| Uv Twist Invert U                           | Applies distortion to the UVs to create directional distortion effect.     |
| Uv Twist Invert V                           | Uses UV tiling and offset to repeat and shift the position of the map.     |
| Strength By Use Custom Data Channel (0 - 3) | Uses UV tiling and offset to repeat and shift the position of the map.     |
| Uv Twist Alpha Use Custom Data              | Uses UV tiling and offset to repeat and shift the position of the map.     |
| Uv Twist Dissolve Use Custom Data           | Uses UV tiling and offset to repeat and shift the position of the map.     |
| Uv Twist                                    | Uses UV tiling and offset to repeat and shift the position of the map.     |
| Use Custom Data 1 Channel (0 - 3)           | Uses UV tiling and offset to repeat and shift the position of the map.     |
<br>
